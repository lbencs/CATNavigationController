												 ...
												 toViewController:(UIViewController *)toVC
{
	if (operation == UINavigationControllerOperationPop) {
		CATPopTransitionAnimation *animation = [[CATPopTransitionAnimation alloc] initWithCompletion:^{
			if ([[CATPageManager shareManager] animationStatus] == CATNavigationPopAnimationDragFinished ||
				...
				[navigationController at_pop];
			}
			[CATPageManager shareManager].animationStatus = CATNavigationPopAnimationPop;
		}];
		animation.navigationController = self.navigationController;
		return animation;
	} else if (operation == UINavigationControllerOperationPush) {
		CATPushTransitionAnimation *animation = [[CATPushTransitionAnimation alloc] init];
		animation.navigationController = self.navigationController;
		return animation;
...
@property (nonatomic, weak, setter=_setPanGestureRecognizer:) UIPanGestureRecognizer *_panGestureRecognizer;
...
- (void)at_viewDidLoad
{
	...
	self._delegate.navigationController = self;
	self.delegate = self._delegate;
...
- (NSArray<UIViewController *> *)at_popToRootViewControllerAnimated:(BOOL)animated
{
	...
	if ([self _useCustomNavigationAnimation]) {
		for (int i = 0; (viewControllers.count > 0) && (i < viewControllers.count - 1); i++) {
			[self at_pop];
		}
		if (!animated) {
			[self at_pop];
...
- (void)at_pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
	if ([self _useCustomNavigationAnimation]) {
		...
		[self at_push:[UIImage at_screenShotImageWithCaptureView:screem]];
	}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
	...
	if ([[otherGestureRecognizer view] isKindOfClass:[UIScrollView class]]) {
		if ([otherGestureRecognizer isKindOfClass:NSClassFromString(@"UIScrollViewPanGestureRecognizer")]) {
			...
			self._panGestureRecognizer = (UIPanGestureRecognizer *)otherGestureRecognizer;
			point = [self._panGestureRecognizer velocityInView:self._panGestureRecognizer.view];
			...
			if (scrollView.contentOffset.x != 0 ||
				...
				NSLog(@"-------------------YES");
				self._panGestureRecognizer.enabled = NO;
				return YES;
			}
		}else{
			return YES;
...
- (void)_handleControllerPop:(UIPanGestureRecognizer *)recognizer
{
	...
	if (recognizer.state == UIGestureRecognizerStateBegan) {
		...
		self._panGestureRecognizer.enabled = YES;
	}
}
...
CATAssociatedProperty(_panGestureRecognizer, _setPanGestureRecognizer:, UIPanGestureRecognizer *);
@end
...
+ (void)load
{
	...
	dispatch_once(&onceToken, ^{
		...
		CATSwizzeMethod([self class],
						...
						@selector(at_presentViewController:animated:completion:));
		CATSwizzeMethod([self class],
						@selector(dismissViewControllerAnimated:completion:),
						@selector(at_dismissViewControllerAnimated:completion:));
	});
}
